...
//!!!!!! HACK to only have Right Red and Left Blue Trials!!!!!!! quick flip below
	bool abortLeftRed = (centerXDeg-g_pSettings->getXOffset()<g_pScotomaMap->getCenterXPos()&&!currentTrial->isCenterRed());
	bool abortRightBlue = (centerXDeg-g_pSettings->getXOffset()>g_pScotomaMap->getCenterXPos()&&currentTrial->isCenterRed());
	bool abortCenter = (centerXDeg-g_pSettings->getXOffset()==0)&&(g_pScotomaMap->getNumCtrLocations()>1);
	if(abortLeftRed||abortRightBlue||abortCenter){
...
void CScotomaMapStimulus::DoTimeout(long mstime) 
{
	...
	msg.Format("Failed, timeout %ld\n", mstime);
	g_pScotomaMapDlg->AddStatusLine((LPCTSTR)msg);

	/* Hack to prevent blank screen from showing for humans. */
	if (g_pScotomaMap->getUseKeyboard()) {
		m_currentPage = 1 - m_currentPage;
		vsgSetDrawPage(vsgVIDEOPAGE, m_currentPage, vsgBACKGROUND);
		if (m_useNoise && g_pScotomaMap->getSameBG()) {
...
CScotomaTrial* CScotomaMapStimulus::GetNextTrial() 
{
	...
	if (g_pScotomaMap->getTrials().size()) {
		...
		if (trial->isPeriphRed()) {
			msg.Append("Red\n");
		}
		else if (trial->isPeriphBlue()) {
			msg.Append("Blue\n");
		}
		else if (trial->isPeriphBlank()) {
			msg.Append("Blank\n");
		}
		else {
			msg.Append("Purple\n");
