...
bool CRFMappingStimulus::DisplayStimulus() 
{
		...
		(m_sequenceTimer.CheckTimer() > m_displayTimeMs)) 
	{
		...
		if (m_useBlanks) {
			...
			etu::AnalogOut(A_OUT_PER_TARG_COLOR,-99.0);
		...
		if(!g_pRFMapping->getUseBinocular()){
			...
			Clear(m_useNoise);
			vsgPresent();
			etu::AnalogOut(A_OUT_CTR_TARG_HORIZ, -99.0);
			etu::AnalogOut(A_OUT_CTR_TARG_VERT, -99.0);
			etu::AnalogOut(A_OUT_CTR_TARG_COLOR,-99.0);
			etu::AnalogOut(A_OUT_PER_TARG_HORIZ, -99.0);
			etu::AnalogOut(A_OUT_PER_TARG_VERT, -99.0);
			etu::AnalogOut(A_OUT_PER_TARG_COLOR,-99.0);
...
void CRFMappingStimulus::DrawStimulus() 
{
	...
	//Commented out to see if it is causing new program not to work
/*
